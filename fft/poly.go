...
//     fmt.Println("newfft settings took time", elapsed)

//     start := time.Now()
//     runtime.GOMAXPROCS(8)
//     var wg sync.WaitGroup
//     var evalsA, evalsB []gmcl.Fr
//     wg.Add(1)
//     go func(){
...
//     wg.Wait()
//     elapsed := time.Since(start)
//     fmt.Println("fft false took time", elapsed)
//     start = time.Now()
	res, _ := fs.FFT(ff.MulVecFr(evalsA, evalsB), true)
//     elapsed = time.Since(start)
//     fmt.Println("fft true took time", elapsed)
	res = res[:(aLen + bLen - 1)]
//     start = time.Now()
	res = PolyCondense(res)
//     elapsed = time.Since(start)
//     fmt.Println("polycondense fft took time", elapsed)
	return res
}
